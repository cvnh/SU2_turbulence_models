/*!
 * \file CTurbWKWSolver.cpp
 * \brief Main subroutines of CTurbWKWSolver class
 * \author L. Cavanha
...
#include "../../include/variables/CTurbWKWVariable.hpp"
#include "../../include/variables/CFlowVariable.hpp"
#include "../../../Common/include/parallelization/omp_structure.hpp"
#include "../../../Common/include/toolboxes/geometry_toolbox.hpp"


CTurbWKWSolver::CTurbWKWSolver(CGeometry *geometry, CConfig *config, unsigned short iMesh)
  ...
  wkwParsedOptions = config->GetWKWParsedOptions();
  ...
  if (iMesh == MESH_0 || config->GetMGCycle() == FULLMG_CYCLE) {
    ...
    if (rank == MASTER_NODE) cout << "Initialize Jacobian structure (WKW model)." << endl;
  ...
  if(wkwParsedOptions.version == WKW_OPTIONS::V2006 || wkwParsedOptions.version == WKW_OPTIONS::V2006m) {
    wkw_const.sigma_k   = 0.6;     
    wkw_const.gamma     = 0.52;    
    wkw_const.beta_0    = 0.0708;  
    wkw_const.Clim      = 0.875;   
    wkw_const.sigma_d0  = 0.125;   
  }
  if(wkwParsedOptions.version == WKW_OPTIONS::V1998 || wkwParsedOptions.version == WKW_OPTIONS::V1998m) {
    wkw_const.sigma_k   = 0.5;     
    wkw_const.gamma     = 0.52;    
    wkw_const.beta_0    = 0.072;   
  }
  if(wkwParsedOptions.version == WKW_OPTIONS::V1988 || wkwParsedOptions.version == WKW_OPTIONS::V1988m) {
    wkw_const.sigma_k   = 0.5;     
    wkw_const.gamma     = 5/9;    
    wkw_const.beta      = 0.075;  
  }
    wkw_const.sigma     = 0.5;    
    wkw_const.beta_star = 0.09;   
  ...
  // -> constants (wkw_const) originally passed as pointer -> performance?
  nodes = new CTurbWKWVariable(kine_Inf, omega_Inf, muT_Inf, nPoint, nDim, nVar, wkw_const, config);
...
void CTurbWKWSolver::Preprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config,
...
void CTurbWKWSolver::Postprocessing(CGeometry *geometry, CSolver **solver_container,
  ...
  for (unsigned long iPoint = 0; iPoint < nPoint; iPoint++) {
    ...
    //strain magnitude => Sij
    const su2double StrainMag = max(nodes->GetStrainMag(iPoint), 1e-12); 
    //average strain magnitude => Sij bar
    const su2double StrainMag_bar = StrainMag - x / 3;
    ...
    //const auto& eddy_visc_var = wkwParsedOptions.version == WKW_OPTIONS::V1994 ? VorticityMag : StrainMag;

    // omega hat is omega tilde in original ref. 
    const su2double omega_hat = max(omega, wkw_const.Clim * sqrt(StrainMag_bar * StrainMag_bar / wkw_const.beta_star));
...
void CTurbWKWSolver::Viscous_Residual(unsigned long iEdge, CGeometry* geometry, CSolver** solver_container,
                                     CNumerics* numerics, CConfig* config) {

  /*--- Define an object to set solver specific numerics contribution. ---*/
  auto SolverSpecificNumerics = [&](unsigned long iPoint, unsigned long jPoint) {
    /*--- Menter's first blending function (only WKW)---*/
...
void CTurbWKWSolver::Source_Residual(CGeometry *geometry, CSolver **solver_container,
...
void CTurbWKWSolver::Source_Template(CGeometry *geometry, CSolver **solver_container, CNumerics *numerics,
                                     CConfig *config, unsigned short iMesh) {
}

void CTurbWKWSolver::BC_HeatFlux_Wall(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
...
void CTurbWKWSolver::SetTurbVars_WF(CGeometry *geometry, CSolver **solver_container,
...
void CTurbWKWSolver::BC_Isothermal_Wall(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
                                        CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
void CTurbWKWSolver::BC_Inlet(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
                              CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
  ...
  for (auto iVertex = 0u; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if (geometry->nodes->GetDomain(iPoint)) {
      ...
      /*--- Set the turbulent variable states. Use free-stream WKW
...
void CTurbWKWSolver::BC_Outlet(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
                               CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
void CTurbWKWSolver::BC_Inlet_MixingPlane(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
                                          CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
void CTurbWKWSolver::BC_Inlet_Turbo(CGeometry *geometry, CSolver **solver_container, CNumerics *conv_numerics,
                                    CNumerics *visc_numerics, CConfig *config, unsigned short val_marker) {
...
void CTurbWKWSolver::SetInletAtVertex(const su2double *val_inlet,
...
su2double CTurbWKWSolver::GetInletAtVertex(su2double *val_inlet,
...
void CTurbWKWSolver::SetUniformInlet(const CConfig* config, unsigned short iMarker) {
  if (config->GetMarker_All_KindBC(iMarker) == INLET_FLOW) {
    for (unsigned long iVertex = 0; iVertex < nVertex[iMarker]; iVertex++) {
